...
# profile_role_requests = db002["profile_role_requests"]
...
#             srj_dict = ast.literal_eval(ldent.entry_to_json())[
#                 "attributes"]["cn"][0]
...
#         ]
#     )
#     search_result_json = None if len(
#         ldap.entries) == 0 else ldap.entries[0].entry_to_json()
...
#         }
#         try:
#             foundUser = users.find_one({"uid": info["uid"]})
#             info["profile_roles"] = foundUser["business_roles"]
#         except:
#             pass
...
#             info["managerID"] = user_data["attributes"]["manager"][0].split(",")[
#                 0].split("=")[1]
...
#                         "mail", "businessCategory", "telephoneNumber"],
...
#     userInfo = getUserAttributes(uid, ["manager"])
#     if userInfo["manager"] == "":
#         return None
#     managerID = get_uid_from_dn(userInfo["manager"])
#     managerInfo = getUserAttributes(managerID, ["mail"])
#     managerMail = ""
#     if managerInfo["mail"] != "":
#         managerMail = managerInfo["mail"]
#     return {
#         "uid": managerID,
#         "mail": managerMail
#     }
...
#         FA2.insert_one({"totp_secret": totp_secret,
#                        "mail": mail, "qr_url": qr_url})
...
#     entry = ldap.search('uid=' + userID + ',' +
#                         config['USER_DN'] + ',' + config['BASE_DN'], '(objectclass=person)')
#     if entry:
#         return True
#     return False
...
#     URL = endPoint + f"/api/v1/dags/{dagInfo['approval']}/dagRuns"
#     try:
#         me = {"username": airflow["value"]["username"],
#               "password": airflow["value"]["password"]}
#         data = "{\"conf\": {\"task_id\":\""+taskid+"\"}, \"dag_run_id\": \"" + \
#             "airflow_"+taskid + str(random.random()) + "\" }"
#         headers = {'Content-Type': 'application/json',
#                    'accept': 'application/json'}
#         results = requests.post(URL, data, auth=(
#             me["username"], me["password"]), headers=headers)
...
# def profile_role_reform(prfs):
#     response = []
#     # reformat database data to a more readable json for airflow-----
#     for pr in prfs:
#         final = []  # final data transformation
#         creation_date = pr["creation_date"]  # get the date
#         datetime_date = creation_date.strftime(
#             "%d/%m/%Y %H:%M:%S")  # change date format
#         pr["creation_date"] = datetime_date  # save change
#         # same thing
#         expiration_date = pr["expiration_date"]
#         datetime_date2 = expiration_date.strftime("%d/%m/%Y %H:%M:%S")
#         pr["expiration_date"] = datetime_date2
#         if pr["type"] == "remove":
#             response.append(pr)
#         else:
#             prns = []  # profile role names
#             for tp in pr["technical_profiles"]:
#                 # get the profile role names
#                 prns.append(tp["profile_role_name"])
#             prns = list(dict.fromkeys(prns))  # delete any duplicate values
#             # print(prns)
#             for prname in prns:
#                 # create final schema
#                 final.append({"profile_role_name": prname,
#                              "technical_profiles": []})
#             # print(final)
#             for tp in pr["technical_profiles"]:  # for each technical profile
#                 for obj in final:  # for each object (a profile role) in final
#                     # if the profile role name match the technical profile role name
#                     if obj["profile_role_name"] == tp["profile_role_name"]:
#                         # then append it to this profile role's technical profiles
#                         obj["technical_profiles"].append(tp)
#             del pr["technical_profiles"]  # remove technical
#             for fi in final:
#                 for fitp in fi["technical_profiles"]:
#                     del fitp["profile_role_name"]
#             pr["profile_roles"] = final
#             response.append(pr)
#     return response
...
#     endPoint = airflow["url"]
#     URL = endPoint + f"/api/v1/dags/{dagInfo['confirm']}/dagRuns"
#     try:
#         me = {"username": airflow["value"]["username"],
#               "password": airflow["value"]["password"]}
...
#                 itpInfo = technical_profiles.find_one(
#                     {"technical_profil_id": itp})
#                 itp_visibility.extend(itpInfo["visibility"])
#                 name = ""
#                 crtct_lvl = ""
#                 app_type_id = ""
...
#                 }
#             )
#         itp_visibility = list(dict.fromkeys(itp_visibility))
#         # print(f"pr_visibility : {itp_visibility}")
#         for ug in userGroups:
#             if ug in itp_visibility:
...
#         approvers["others"].extend(others)
#         token = encode_token(
...
#         if req["type_of_request"] == 'ask':
#             profile_roles_start_date = req["profile_roles_start_date"]
#             profile_roles_end_date = req["profile_roles_end_date"]
#             tasks.insert_one(
#                 {
...
#                     "creation_date": now,
#                     "expiration_date": exp,
#                     "profile_roles_start_date": format(datetime.strptime(profile_roles_start_date, "%d/%m/%Y"), "%d/%m/%Y %H:%M:%S"),
#                     "profile_roles_end_date": format(datetime.strptime(profile_roles_end_date, "%d/%m/%Y"), "%d/%m/%Y %H:%M:%S"),
...
#     approversGroupDN = f'cn={name}_approvers,' + \
#         config['GROUP_DN'] + "," + config['BASE_DN']
...
#         return jsonify({
#             "status": "failed",
#             "message": f"Insufficient access rights, you have not role admin"
#         }), 401
#     attr = {
#         'cn': name,
...
#     AUTHORIZED = "admin" in this_user_roles
#     if not AUTHORIZED:
#         return jsonify({
#             "status": "failed",
#             "message": f"Insufficient access rights, you have not role admin to delete this role"
#         }), 401
...
#                         'posixAccount', 'shadowAccount', 'person']

#         # check if role existing
#         search_dn = config['GROUP_DN'] + ',' + config['BASE_DN']
#         for eachRole in user["role"]:
#             tr = ldap.search(
#                 search_dn,
#                 f'(&(objectclass=*)(cn={eachRole}))',
#                 attributes=['cn']
#             )
#             if not tr:
#                 return jsonify({
#                     "status": "failed",
#                     "message": f"The role {eachRole} of the user isn't defined"
#                 }), 400
...
#             users_group_list = [    #default roles
#                 readonly_group,
#                 getGroupDn(config['ROLE_VIEWER']),
#                 getGroupDn(config['TECHNICAL_VIEWER'])
#             ]
#             for a_role in user["role"]:
#                 users_group_list.append(getGroupDn(a_role)) 
#             users_group_list = list(dict.fromkeys(users_group_list))
#             for eachRole in users_group_list:
#                 addUsersInGroups(ldap, user_dn, eachRole)
...
#             {"status": "success", "data": data_to_display}, indent=2)
...
#             except:
#                 user_info[k] = ""
#         found_user = users.find_one({"uid": uid})
#         user_info["2fa"] = FA2info.find_one({"uid": uid})["2fa"]
#         user_info["profile_roles"] = found_user["business_roles"]
#         return jsonify({
#             "status": "success",
...
#             req["uid"], ["cn", "sn", "businessCategory", "telephoneNumber", "mail", "uid"])
...
# def change_password():
#     validated = validation(required_keys=["newPassword", "oldPassword"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
...
#     validated = validation(required_keys=["user_uid", "newPassword"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#         return jsonify({
#             "status": "success",
#             "message": "change password request is now processing.."
#         })
#     except:
#         return jsonify({
#             "status": "failed",
...
#             tasks.insert_one({"taskid": taskid, "type": "rights_request",
#                              "user_uid": req["user_uid"], "group_name": req["group_name"]})
#             Thread(target=mail_sender, args=(
#                 manager["mail"], objet, message,)).start()
...
#         if action == "grant":
#             result = addUsersInGroups(ldap, getUserDn(
#                 uid), getGroupDn(taskInfo["group_name"]))
#             if not result:
...
#             Thread(target=mail_sender, args=(
#                 user_mail, "Rights request", message,)).start()
...
# def createApp():
#     validated = validation(required_keys=["name", "type"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights, you have not role admin"
#             }), 401
#         appid = str(ObjectId())
#         name = req["name"]
#         app_type = req["type"]
...
# @ORGANIGRAM_REQUEST.route('/application/update', methods=['PUT'])
# def updateApp():
#     validated = validation(required_keys=["app_id"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             if not has_role(["admin", config["ROLE_MGR"], config["ROLE_VIEWER"]], "one"):
#                 return jsonify({
#                     "status": "failed",
#                     "message": "Insufficient access rights. Not allowed to view profil role"
#                 }), 401
#             user_groups = getUserGroup(getUserDn(userid), uid=userid)
#             data = []
#             allRoles = profile_roles.find()
#             has_admin_super_power = has_role(["admin"], "one")
#             for doc in allRoles:
#                 if has_admin_super_power:
#                     data.append({
#                         "profil_role_id": doc["profil_role_id"],
#                         "name": doc["name"],
#                         "id_technical_profil": doc["id_technical_profil"],
#                         "visibility": doc["visibility"],
#                         "criticity_level": doc["criticity_level"],
#                         "app_type_id": doc["app_type_id"]
#                     })
#                 else:
#                     check2 = any(
#                         item in user_groups for item in doc["visibility"])
#                     if check2:
#                         data.append({
#                             "profil_role_id": doc["profil_role_id"],
#                             "name": doc["name"],
...
# def createRole():
#     validated = validation(required_keys=["app_type_id", "name", "visibility"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
#         if not has_role(["admin", config["ROLE_MGR"]], "one"):
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to create profil role"
#             }), 401
...
#         gen_criticity_level = 0
#         for ptid in tpInfo[0]:
#             pt_criticity_level = technical_profiles.find_one(
#                 {"technical_profil_id": ptid})["criticity_level"]
#             if pt_criticity_level > gen_criticity_level:
#                 gen_criticity_level = pt_criticity_level
#         newRole = {
...
#             "criticity_level": gen_criticity_level
#         }
...
# def updateRole():
#     validated = validation(required_keys=["profil_role_id"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to update profil role"
#             }), 401

#         profil_role = profile_roles.find_one(
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to delete profil role"
#             }), 401
#         toDelete = profile_roles.find_one(
#             {"profil_role_id": req["profil_role_id"]})
#         if not toDelete:
#             return jsonify({"status": "failed", "message": "Profil role not found"}), 404
...
#         try:
#             userid = get_userid_by_token()
#             if not has_role(["admin", config["TECHNICAL_MGR"], config["TECHNICAL_VIEWER"]], "one"):
#                 return jsonify({
#                     "status": "failed",
#                     "message": "Insufficient access rights. Not allowed to view technical profil"
#                 }), 401
#             user_groups = getUserGroup(getUserDn(userid), uid=userid)
#             data = []
...
#     validated = validation(
#         required_keys=["visibility", "app_type_id", "criticity_level", "name"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
#         if not has_role(["admin", config["TECHNICAL_MGR"]], "one"):
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to create technical profil"
#             }), 401
#         if len(req["visibility"]) == 0:
#             return jsonify({
#                 "status": "failed",
...
#         if check:
#             return jsonify({"status": "failed", "message": "This technical_profile already exists."}), 409
#         TPiD = str(ObjectId())
...
#     validated = validation(required_keys=["technical_profil_id"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to update technical profil"
#             }), 401

#         technical_profil = technical_profiles.find_one(
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to delete technical profil"
#             }), 401
#         toUpdate = technical_profiles.find_one(
#             {"technical_profil_id": req["technical_profil_id"]})
#         if not toUpdate:
#             return jsonify({"status": "failed", "message": "technical_profil does not exist"}), 400
#         technical_profiles.delete_one(
#             {"technical_profil_id": req["technical_profil_id"]})
...
#     validated = validation(
#         required_keys=["business_role_ids", "type_of_request"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             }), 400
#         if req["type_of_request"] == "ask":
#             if not isErrorKey(req, "profile_roles_start_date"):
#                 return jsonify({
#                     "status": "failed",
#                     "message": "Profile role start date is required"
#                 })
#             if not isErrorKey(req, "profile_roles_end_date"):
#                 return jsonify({
#                     "status": "failed",
#                     "message": "Profile role end date is required"
#                 })
#         business_role_ids = req["business_role_ids"]
#         for bri in business_role_ids:
#             profile_role = profile_roles.find_one({"profil_role_id": bri})
...
# def profileRoleAdd():
#     validated = validation(required_keys=["userid", "business_role_ids"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             return jsonify({
#                 "status": "failed",
#                 "message": "Insufficient access rights. Not allowed to add profil role"
#             }), 401
#         manager = getUserManager(userid)
#         approvers = []
#         if manager is not None:
...
#     validated = validation(required_keys=["business_role_ids", "userid"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#     try:
#         userid = get_userid_by_token()
#         AUTHORIZED = has_role(["admin"], "one")
#         response = []
#         prfs = profile_role_requests.find(
#             {
#                 "approver_uid": userid,
#                 "state": {"$ne": "done"}
#             },
...
#         response = profile_role_reform(prfs)
#         data = response
#         if AUTHORIZED:
#             others_prfs = profile_role_requests.find(
#                 {
#                     "state": {"$ne": "done"},
...
#                 {"_id": 0, "token": 0}
#             )
#             response2 = profile_role_reform(others_prfs)
#             others = response2
#             # for pr in others_prfs:
#             #     creation_date = pr["creation_date"]
#             #     datetime_date = creation_date.strftime("%d/%m/%Y %H:%M:%S")
#             #     pr["creation_date"] = datetime_date
...
#         AUTHORIZED = has_role(["admin"], "one")
#         response = []
#         tprf = profile_role_requests.find({"approver_uid": userid, "state": "done"}, {
...
#         data = response
#         if AUTHORIZED:
#             tprf = profile_role_requests.find(
#                 {"state": "done"},
#                 {"_id": 0, "token": 0}
#             )
#             to_remove = []
...
#         response = []
#         tprf = profile_role_requests.find(
#             {"approver_uid": userid, "state": {"$ne": "done"}, "token": token},
#             {"_id": 0, "approver_uid": 0}
#         )
#         response = profile_role_reform(tprf)
#         default_str = json.dumps(
#             {
#                 "status": "success",
#                 "message": "",
...
# @ORGANIGRAM_REQUEST.route('/user/profil_role/grant_deny', methods=['POST'])
# def profileRoleGrantDeny():
#     validated = validation(required_keys=["token"])
#     if not validated[0]:
#         return validated[1]
#     req = validated[1]
#     try:
...
#             if found_task["type_of_request"] == "remove":
#                 condition = ar["state"] != "approved" and ar["state"] != "rejected"
#             else:
#                 for ar2 in ar["technical_profiles"]:
#                     condition = ar2["state"] != "approved" and ar2["state"] != "rejected"
#             if condition:
