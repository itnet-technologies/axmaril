    ...
    try:
        ...
        if request.content_type == "application/json":
            ...
            try:
                ...
                already_existing_safe = safes.find_one({"safe_id" : safe_id, "owner_uid" : owner_uid})
                ...
                if not already_existing_safe:
                    safe_access_info = safe_access(safe_id, owner_uid)
                    if not safe_access_info[0]:
                        return jsonify({"message":"safe not found", "status": "failed"}), 404
                    else:
                        if not safe_access_info[2]["write"]:
                            return jsonify({
                                "status": "failed",
                                "message":"safe access denied"
                            }), 400
                ...
                if "exp_time" in req:
                    ...
                    expiration_date_time = creation_date_time + timedelta(seconds = req["exp_time"])
            ...
            if not isErrorKey(data, "safe_id"):
                return jsonify({
                    "status" : "failed",
            ...
            if isErrorKey(secret_infos, "file_path"):
                del secret_infos["file_path"]
            if isErrorKey(secret_infos, "_id"):
                del secret_infos["_id"]
            if isErrorKey(secret_infos, "secret"):
                del secret_infos["secret"]
            return jsonify({
                "status" : "success",
                "message" : "secret file successfully created",
                "data" : secret_infos
            })
    except:
        print(traceback.format_exc())
